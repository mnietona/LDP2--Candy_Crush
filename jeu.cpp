...
class Rectangle {
 ...
 public:
  Rectangle(Point center, int w, int h,
            Fl_Color frameColor = fl_rgb_color(214,214,214),
            Fl_Color fillColor = fl_rgb_color(214,214,214));
  void draw();
...
Cell::Cell(Point center, int w, int h, int r, Fl_Color color_candy): 
  ...
  r(center, w, h, fl_rgb_color(214,214,214), fl_rgb_color(214,214,214)), 
  selected(false),
  ...
  for (int x = 0; x<9; x++) {
    ...
    for (int y = 0; y<9; y++)
    {
      ...
      switch(random_number)
      {
        ...
        case 4:
        color_candy = fl_rgb_color(255,140,0);
        break;
      }
      cells[x].push_back({{72*x+50, 72*y+62}, 70, 70, 25, color_candy});
    }
  }
...
void Canvas::SwitchTwoCells(Cell c, Cell* n)
{
  ...
  swap( 
  cells[(c.getCenter().x-50)/72][(c.getCenter().y - 62)/72],
  cells[(n->getCenter().x-50)/72][(n->getCenter().y - 62)/72]
  );
...
class Image {
  ...
  int w, h;
  public:
  Image(Point center, int w, int h);
  void draw();

};

Image::Image(Point center, int w, int h):
...
void Image::draw(){
    ...
    */
    Fl_PNG_Image* candy = new Fl_PNG_Image("bleu.png");
    Fl_Box *box = new Fl_Box(center.x-w/2, center.y-h/2, w, h);
    box->image(candy);
    

    //fl_draw_box(FL_FLAT_BOX, center.x-w/2, center.y-h/2, w, h,FL_BLUE);
    //fl_draw_box(FL_BORDER_FRAME, center.x-w/2, center.y-h/2, w, h, FL_RED);
...
class MainWindow : public Fl_Window {
  ...
  Game newGame;
 public:
  MainWindow(Game newGame) : Fl_Window(100, 100, windowWidth, windowHeight, "Candy Crush") {
...
void MainWindow::drawWelcomeBoard()
{
  ...
  //Text welcome("CANDY CRUSH", {windowWidth/2, windowHeight/2}, 30, pink);
  //Text author("SALUT FAUT JOUER", {(windowWidth/2)-50,(windowHeight/2)-50}, 10, FL_BLACK);
  //author.draw();
  //welcome.draw();

  Image test({windowWidth/2, windowHeight/2},30,30);
  test.draw();
}
